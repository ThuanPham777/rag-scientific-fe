...
import { Sparkles, FileText, Quote, ArrowRight } from "lucide-react";

// --- Types (giữ nguyên để tránh lỗi TS) ---
type BBox = {
  x1: number; y1: number; x2: number; y2: number;
  layout_width?: number; layout_height?: number;
};
...
type TextContext = {
  ...
  snippet?: string;
  metadata?: any;
  locator?: { bbox?: BBox | null; page_label?: number | null; [key: string]: any };
};
...
type Props = {
  ...
  onJumpToSource?: (params: { pageNumber: number; rect: { top: number; left: number; width: number; height: number } }) => void;
};

export default function SummaryView({ summaryData, onJumpToSource }: Props) {
  const answerText = summaryData?.answer ?? summaryData?.summary ?? "No summary available yet.";
  ...
  const sourceMap = useMemo(() => {
    ...
    summaryData?.context?.texts?.forEach((t) => {
      ...
      if (!id) return;
      const pageNumber = typeof pageLabel === "number" ? pageLabel : Number(pageLabel) || undefined;
  ...
  const renderFormattedText = (fullText: string) => {
    ...
    return paragraphs.map((para, i) => {
      ...
      while ((match = regex.exec(para)) !== null) {
        if (match.index > lastIndex) nodes.push(para.slice(lastIndex, match.index));
        const id = match[1];
        nodes.push(
          <button
            key={`${id}-${match.index}`}
      ...
      return (
        <p key={i} className="mb-4 text-gray-800 leading-7 text-[15px] text-justify">
          {nodes}
        </p>
      );
    });
    ...
    <div className="h-full overflow-y-auto bg-gray-50/50">
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Executive Summary</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-sm font-medium text-gray-500">AI-Generated Analysis</span>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {summaryData.context.texts.map((t) => {
                 const pageInfo = t.metadata?.page_label ? `Page ${t.metadata.page_label}` : "Unknown Page";
                 const title = t.metadata?.section_title ?? t.metadata?.type ?? "Section text";
